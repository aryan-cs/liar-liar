...
\label{eq:effective}
\end{equation}
where $\sigma(z) = \sqrt{d^{-1}\|z\|_2^2+\varepsilon}$. Projecting against raw $\WU$ rows, as in prior unembedding-orthogonal constructions, leaves a residual direct effect through the gain and the rank-one term. We average $J_{\gamma}$ over a calibration set of readout points (Section~\ref{sec:setup}) and write $\WUt^{*}$ for the resulting matrix.
